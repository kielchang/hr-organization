...
export function BpmnListPage() {
  ...
  // Instant health check for all processes (memoized)
  const healthMap = useMemo(
    () => new Map(store.processes.map((p) => [p.id, analyzeProcessHealth(p, data)])),
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" className="gap-2" onClick={() => navigate('/bpmn/impact')}>
            <BarChart2 className="size-4" />影響分析
          </Button>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {health?.severity === 'critical' && (
                      <Badge variant="destructive" className="text-[9px] gap-0.5">
                        <AlertCircle className="size-2.5" />核准斷裂
                      </Badge>
                    )}
                    {health?.severity === 'warning' && (
                      <Badge variant="warning" className="text-[9px] gap-0.5">
                        <AlertTriangle className="size-2.5" />單點風險
                      </Badge>
                    )}
                  </div>
                  {p.description && (
